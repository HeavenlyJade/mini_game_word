...
function LevelRewardNodeInitializer.OnTriggerBoxTouched(actor, uniqueId, rewardNode, configName, mapName, triggerBox)
    ...
    -- 【新增】获取本地玩家的actor对象
    local localActor = gg.getClientLocalPlayer()

    if not localActor then
        --gg.log("[LevelRewardNodeInitializer] 错误：本地玩家角色不存在")
        return
    end

    -- 【新增】检查触发的actor是否与本地玩家的actor一致
    if actor ~= localActor then
        --gg.log("[LevelRewardNodeInitializer] 警告：触发的actor与本地玩家actor不一致，忽略触发")
        --gg.log("  触发的actor:", actor, "本地玩家actor:", localActor)
        return
    end

    -- 【新增】验证玩家ID一致性
    if playerId ~= localActor.UserId then
        --gg.log("[LevelRewardNodeInitializer] 错误：玩家ID不一致，触发的actor ID:", playerId, "本地玩家ID:", localActor.UserId)
        return
    end
