...
function RaceGameAction:OnStart(data)
    ...
    local jumpDuration = data.jumpDuration
    ...
    self.stopJumpTimer = ScheduledTask.AddDelay(2, "RaceGameAction_StopJump", function()
        if actor then
    ...
    -- 8. 添加强制落地机制 - 如果重力为0，在一定时间后恢复重力
    if gravity == 0 then
        local forceGravityDelay = math.min(recoveryDelay * 0.8, 45) -- 80%的时间后或45秒后恢复重力
...
function RaceGameAction:OnEnd()
    ...
    if actor then
        ...
        actor.Gravity = self.originalGravity -- 核心修复：恢复原始重力
