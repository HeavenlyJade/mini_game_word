...
function BonusCalculator.CalculateOtherBonuses(player, baseValue, otherBonuses, targetName)
    ...
    for _, bonusType in ipairs(otherBonuses) do
        if bonusType == "宠物" then
            ...
            for itemName, bonusData in pairs(petBonuses) do
                ...
                local mappedTargetVariable = bonusData.targetVariable
                if bonusData.targetVariable and AttributeMapping.IsAttributeVariable(bonusData.targetVariable) then
                    mappedTargetVariable = AttributeMapping.GetCorrespondingStat(bonusData.targetVariable)
                end
                
                local isMatch = bonusData.targetVariable == targetName or
                               mappedTargetVariable == targetName or
        ...
        elseif bonusType == "翅膀" then
            ...
            for itemName, bonusData in pairs(wingBonuses) do
                --gg.log(" bonusData.targetVariable", bonusData,targetName,itemName)
                -- 转换 bonusData.targetVariable
                local mappedTargetVariable = bonusData.targetVariable
                if bonusData.targetVariable and AttributeMapping.IsAttributeVariable(bonusData.targetVariable) then
                    mappedTargetVariable = AttributeMapping.GetCorrespondingStat(bonusData.targetVariable)
    ...
    -- gg.log("其他加成", totalBonus, bonusDescriptions,baseValue, otherBonuses, targetName)
    return totalBonus, bonusDescriptions
end
...
---@param otherBonuses table 其他加成类型列表
---@param targetName string 目标名称
---@param calcType string 计算类型（""属性计算或"玩家变量计算"）
---@return number, string finalValue, bonusInfo
function BonusCalculator.CalculateAllBonuses(player, baseValue, playerStatBonuses, playerVariableBonuses, otherBonuses, targetName,calcType)
    ...
    if otherBonuses and type(otherBonuses) == "table" and #otherBonuses > 0 then
        local calcValue = currentValue
        if calcType == "属性计算" then
            calcValue = baseValue
        elseif calcType == "玩家变量计算" then
            calcValue = currentValue
            
        end

        local otherBonusValue, otherDescriptions = BonusCalculator.CalculateOtherBonuses(player, calcValue, otherBonuses, targetName)
