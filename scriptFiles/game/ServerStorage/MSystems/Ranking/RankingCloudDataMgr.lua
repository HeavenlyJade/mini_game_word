...
-- 新增：排行榜数据缓存
local rankingDataCache = {} ---@type table<string, table>

--- 新增：获取或初始化排行榜数据缓存
---@param rankType string 排行榜类型
---@return table 缓存实例
local function GetOrInitRankingCache(rankType)
    if not rankingDataCache[rankType] then
        rankingDataCache[rankType] = {
            data = {},              -- {[uin] = {playerName, score}}
        }
    end
    return rankingDataCache[rankType]
end

--- 新增：从云端初始化缓存
---@param rankType string 排行榜类型
---@return boolean 是否成功
function RankingCloudDataMgr.InitCacheFromCloud(rankType)
    local cache = GetOrInitRankingCache(rankType)
    
    --gg.log("开始从云端初始化排行榜缓存", rankType)
    
    -- 获取云端TOP数据
    local cloudData = RankingCloudDataMgr.GetTopRankingData(rankType, 100)
    if not cloudData then
        --gg.log("云端缓存初始化失败：无法获取数据", rankType)
        return false
    end
    ...
    for _, entry in pairs(cloudData) do
        if entry.key and entry.name and entry.value then
            local uin = tonumber(entry.key)
            if uin then
                cache.data[uin] = {
    ...
    --gg.log("云端缓存初始化完成", rankType, "缓存玩家数:", #cloudData)
    return true
end
...
function RankingCloudDataMgr.GetPlayerScore(rankType, uin, playerName)
    ...
    local uinNum = tonumber(uin)
    if not uinNum then
        --gg.log("获取玩家分数失败：UIN无效", rankType, uin)
        return -1
    end
    
    -- 尝试从缓存获取
    local cache = GetOrInitRankingCache(rankType)
    local playerData = cache.data[uinNum]
    if playerData then
        ----gg.log("从缓存获取玩家分数", rankType, uin, playerName, playerData.score)
    ...
    if result and type(result) == "number" then
        ...
        ----gg.log("从云端获取玩家分数并缓存", rankType, uin, playerName, result)
        return result
    else
...
--- 修改：BatchSafeUpdatePlayerScore - 改为缓存优先模式
---@param rankType string 排行榜类型
...
function RankingCloudDataMgr.BatchSafeUpdatePlayerScore(rankType, updates, forceUpdate)
    ...
    -- 批量更新缓存
    local cacheChangeCount = RankingCloudDataMgr.BatchUpdateCache(rankType, updates)
    ...
    if cloudStore then
        for _, updateData in pairs(updates) do
            if updateData and #updateData >= 3 then
                local uin = updateData[1]
                local playerName = updateData[2]
                local score = updateData[3]
                
                local result = cloudStore:SetValue(tostring(uin), playerName, math.floor(score))
                local success = false
                if type(result) == "number" then
                    success = (result == 0)
                elseif type(result) == "boolean" then
    ...
    for i, updateData in pairs(updates) do
        if updateData and type(updateData) == "table" and #updateData >= 3 then
            ...
            if uinNum and cache.data[uinNum] then
                success = true
                finalScore = cache.data[uinNum].score
            end
            ...
            table.insert(results, {
                ...
                oldScore = oldScore,
                newScore = finalScore,
                scoreChanged = (oldScore ~= finalScore)
            })
        else
            table.insert(results, {
                index = i,
    ...
    --gg.log("批量缓存更新完成", rankType, "总数:", #updates, "缓存变化:", cacheChangeCount, "云端更新:", successCount)
...
--- 新增：批量更新缓存数据
---@param rankType string 排行榜类型
---@param updates table 更新数据 {{uin, playerName, score}, ...}
---@return number 实际有变化的更新数量
function RankingCloudDataMgr.BatchUpdateCache(rankType, updates)
    ...
    for _, updateData in pairs(updates) do
        if updateData and #updateData >= 3 then
            ...
            local uinNum = tonumber(uin)
            if uinNum and type(newScore) == "number" then
                local intScore = math.floor(newScore)
                local oldData = cache.data[uinNum]
                local oldScore = oldData and oldData.score or 0
                
                -- 只有分数真正提高时才更新
                if intScore > oldScore then
                    cache.data[uinNum] = {
                        playerName = playerName,
                        score = intScore
                    }
                    ...
                    --gg.log("缓存更新", rankType, uin, playerName, oldScore, "->", intScore)
                end
            end
        end
    end
    
    --gg.log("批量缓存更新完成", rankType, "处理:", #updates, "变化:", changeCount)
    return changeCount
end
