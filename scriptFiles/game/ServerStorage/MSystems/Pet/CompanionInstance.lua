...
function CompanionInstance:GetItemBonuses()
    ...
    if not self.companionTypeConfig or not self.companionTypeConfig.CalculateCarryingEffectsByStarLevel then
        --gg.log(string.format("[CompanionInstance调试] GetItemBonuses: 配置无效 %s (Slot %d)", self:GetName(), self.slotIndex))
        return {}
    end
    ...
    --gg.log(string.format("[CompanionInstance调试] GetItemBonuses for %s (Slot %d, Star %d) - Received effects:", self:GetName(), self.slotIndex, starLevel))
    --gg.log(allEffects)
    ...
    for variableName, effectData in pairs(allEffects) do
        ...
        if effectData.bonusType == "物品" and effectData.itemTarget then
            ...
            if not itemBonuses[itemName] then
                itemBonuses[itemName] = { fixed = 0, percentage = 0, itemTarget = itemName }
            end
            ...
            if effectData.isPercentage then
                ...
                --gg.log(string.format("[CompanionInstance调试] 添加物品百分比加成: %s +%d%%", itemName, bonusValue * 100))
            else
        ...
        if effectData.bonusType == "玩家变量" and effectData.targetVariable then
            ...
            if effectData.isPercentage then
                ...
                --gg.log(string.format("[CompanionInstance调试] 添加玩家变量百分比加成: %s +%d%%", targetVar, bonusValue * 100))
            else
    ...
    --gg.log(string.format("[CompanionInstance调试] GetItemBonuses for %s (Slot %d) - Final item bonuses:", self:GetName(), self.slotIndex))
    --gg.log(itemBonuses)
