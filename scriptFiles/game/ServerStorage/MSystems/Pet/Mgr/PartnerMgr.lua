...
---@param slotIndex number 槽位索引
---@return boolean, string|nil
function PartnerMgr.DeletePartner(uin, slotIndex)
    local partnerManager = PartnerMgr.GetPlayerPartner(uin)
    if not partnerManager then
        return false, "玩家伙伴数据不存在"
    end

    local success, errorMsg = partnerManager:DeletePartner(slotIndex)
...
function PartnerMgr.TogglePartnerLock(uin, slotIndex)
    local partnerManager = PartnerMgr.GetPlayerPartner(uin)
    if not partnerManager then
        return false, "玩家伙伴数据不存在", nil
    end

    return partnerManager:TogglePartnerLock(slotIndex)
end
...
function PartnerMgr.AutoEquipBestEffectPartner(uin, equipSlotId, excludeEquipped)
    local partnerManager = PartnerMgr.GetPlayerPartner(uin)
    if not partnerManager then
        return false, "玩家伙伴数据不存在", nil
    end
    
    local success, errorMsg, slotIndex = partnerManager:AutoEquipBestEffectCompanion(equipSlotId, excludeEquipped)
...
---@param excludeEquipped boolean|nil 是否排除已装备的伙伴
---@return table 装备结果
function PartnerMgr.AutoEquipAllBestEffectPartners(uin, excludeEquipped)
    local partnerManager = PartnerMgr.GetPlayerPartner(uin)
    if not partnerManager then
        return {success = false, errorMsg = "玩家伙伴数据不存在"}
    end
    
    local results = partnerManager:AutoEquipAllBestEffectCompanions(excludeEquipped)
...
function PartnerMgr.GetPartnerEffectRanking(uin, limit)
    local partnerManager = PartnerMgr.GetPlayerPartner(uin)
    if not partnerManager then
        return nil, "玩家伙伴数据不存在"
    end
    
    local ranking = partnerManager:GetEffectValueRanking(limit)
    return ranking, nil
end
