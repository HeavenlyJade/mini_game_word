...
-- 记录每个玩家所在的模式实例ID
-- key 为 player.UserId (number), value 为 instanceId (string)
...
function GameModeManager:Update(dt)
    -- for instanceId, mode in pairs(self.activeModes) do
    --     mode:Update(dt)
    -- end
end
...
game:GetService("Players").PlayerRemoving:Connect(function(player)
    ...
    local mPlayer = MServerDataManager.getPlayerByUin(player.UserId)
    if mPlayer then
        GameModeManager:RemovePlayerFromCurrentMode(mPlayer)
    else
        -- 如果此时已经找不到MPlayer实例（可能已被清理），
        -- 我们需要一种备用方式来从比赛中移除该玩家的记录
        local instanceId = GameModeManager.playerModes[player.UserId]
        if instanceId then
             GameModeManager.playerModes[player.UserId] = nil
             gg.log(string.format("游戏模式管理器: 玩家(UIN: %d)的实例已不存在，但已成功从比赛记录中移除。", player.UserId))
        end
    end
end)
